...
- Bound your mailboxes if a peer can outrun them. `CreateMailbox` and `RegisterGenServer` default to
  `capacity: 0`, which is unbounded — the same as an Erlang process mailbox, but the emulator has
  `max_heap_size` behind it and this does not, so a peer sending to a registered name faster than
  the handler drains it grows the inbox until the process dies. A positive `capacity` bounds it by
  dropping the **oldest** message, not by refusing the newest.
...
The codec tests assert against byte vectors captured from a real Erlang runtime
